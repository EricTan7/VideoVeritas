...
VIDEO_MAX_TOKEN_NUM = int(os.environ.get('VIDEO_MAX_TOKEN_NUM', 768))
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    task_type = ele.get("task", None)
    if task_type is None or task_type in ["aigc"]:
        ...
        if duration_str:
            try:
                ...
                if duration > 0:
                    ele['video_start'] = 0.0
                    ele['video_end'] = duration
                    # logger.info(f"Environment variable DURATION={duration}s is set. Forcing video to start at 0.0s and end at {duration}s.")
            except (ValueError, TypeError):
                logger.warning(f"Environment variable DURATION='{duration_str}' is not a valid number. Ignoring it.")
    else:
        # perception task：use fps=2.0
        ele["fps"] = 2.0
        ele['video_start'] = 0.0
        ele['video_end'] = 10.0
