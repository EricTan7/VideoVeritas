...
def _build_transform_aug_woresize(input_size):
    trans = []
    # choices = ['resize_then_crop', 'random_flip', 'random_rotate','blur','brightness_contrast','colorjitter','compression']
    choices = ['random_flip', 'random_rotate','blur','brightness_contrast','colorjitter','compression']
    if 'resize' in choices:
        trans += [A.Resize(height=input_size, width=input_size, interpolation=cv2.INTER_CUBIC)]
    if 'random_crop' in choices:
        trans += [A.RandomCrop(height=input_size, width=input_size, p=1)]
...
def transform_image(image, input_size=448, max_num=12, aug=False, pre_resize=False, pre_resize_size=None, aug_lq_only=False, aug_hq_only=False, pre_aug=False, gaussian_sigma=None, jpeg_quality=None, resize_robust=None):
    ## pre resize逻辑
    if pre_resize:
        # image = image.resize((input_size, input_size))
        image = image.resize((pre_resize_size, pre_resize_size))
    if pre_aug:
        transform = _build_transform_aug_woresize(input_size=input_size)
        image = transform(image=np.array(image.convert("RGB")))['image']
    ...
    if resize_robust:   # resize to 0.5
        orig_width, orig_height = image.size
        image = image.resize((orig_width//2, orig_height//2))
    if gaussian_sigma is not None:
        image = np.array(image)
        gaussian_filter(image[:,:,0], output=image[:,:,0], sigma=gaussian_sigma)
        gaussian_filter(image[:,:,1], output=image[:,:,1], sigma=gaussian_sigma)
        gaussian_filter(image[:,:,2], output=image[:,:,2], sigma=gaussian_sigma)
        image = Image.fromarray(image)
    if jpeg_quality is not None:
        out = BytesIO()
        image.save(out, format='jpeg', quality=jpeg_quality) # ranging from 0-95, 75 is default
    ...
    if aug:
        if aug_lq_only and image.size[0] > 512:
            transform = _build_transform(input_size=input_size)
            pixel_values = [transform(image) for image in images]
        elif aug_hq_only and image.size[0] < 512:
            transform = _build_transform(input_size=input_size)
            pixel_values = [transform(image) for image in images]
        else:
...
def load_video_internvl_self(video: Union[str, bytes], bound=None, duration=2.0, target_fps=8, use_timestamp=False):
    ## 支持指定duration, fps
    from decord import VideoReader, cpu
    video_io = load_file(video)
    vr = VideoReader(video_io, ctx=cpu(0), num_threads=1)
    ...
    start_idx = 0
    end_idx = min(len(vr)-1, int(duration*fps))
    num_frames_to_extract = int(duration*target_fps)
    frame_indices = np.linspace(start_idx, end_idx, num=num_frames_to_extract, dtype=int).tolist()
    ...
    if use_timestamp:
        timestamp = [round(frame_index / fps, 1) for frame_index in frame_indices]
