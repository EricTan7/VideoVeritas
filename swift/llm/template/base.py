...
## custom: 根据tag分割文本，添加mask
def split_response_by_tags(
    response: str, 
    start_tag_text: str, 
    end_tag_text: str,
):
    # find targeted tags
    start_tag_char_idx = response.find(start_tag_text)
    end_tag_char_idx = response.find(end_tag_text)

    if start_tag_char_idx == -1 or end_tag_char_idx == -1:
        # if not found, do not perform masking
        print(f"Warning: Tags for '{start_tag_text}' or '{end_tag_text}' not found in response.")
        return [{'text': response, 'type': ContextType.RESPONSE}]

    range_start_idx = start_tag_char_idx
    range_end_idx = end_tag_char_idx + len(end_tag_text)

    if range_start_idx >= range_end_idx:
        print("Warning: Start tag appears after end tag.")
        return [{'text': response, 'type': ContextType.RESPONSE}]
    segments = []
    
    # Before start tag: mask
    prefix_text = response[:range_start_idx]
    if prefix_text:
        segments.append({'text': prefix_text, 'type': ContextType.RESPONSE_MASK})
        
    # From start-end tag
    main_text = response[range_start_idx:range_end_idx]
    if main_text:
        segments.append({'text': main_text, 'type': ContextType.RESPONSE})

    # After end tag: mask
    suffix_text = response[range_end_idx:]
    if suffix_text:
        segments.append({'text': suffix_text, 'type': ContextType.RESPONSE_MASK})
...
class Template(ProcessorMixin):
    ...
    def forward_context(self, model, inputs):
        ...
        # print("*")
        # print(inputs)  inputs: dict_keys(['input_ids', 'labels', 'attention_mask', 'pixel_values'])
        return nullcontext()
    ...
    def _concat_context_list(
            ...
            # custom
            mask_start_tag: Optional[str] = None,
            mask_end_tag: Optional[str] = None) -> None:
        ...
        for context in context_list:
            if isinstance(context, str):
                if '{{RESPONSE}}' == context:
                    ...
                    ## custom
                    if mask_start_tag and mask_end_tag:
                        # split response if tags are given
                        response_segments = split_response_by_tags(response, mask_start_tag, mask_end_tag)
                        for segment in response_segments:
                            if segment['text']: # 确保不添加空字符串
                                res_context_list.append(segment['text'])
                                res_context_type.append(segment['type'])
                    else:
    ...
    def _encode_context_list(self,
        ...
        for i, (context, loss_weight) in enumerate(zip(context_list, loss_scale_list)):
            ...
            if loss_scale_list[i] > 0.0:
                labels += token_list
            elif loss_scale_list[i] < 0.0:
                labels += token_list  # need to maintain org lables for nll loss
            else:
    ...
    def _swift_encode(self, inputs: StdTemplateInputs):
        ...
        # res_context_list ['system xxx']  res_context_types ['others']
        ...
        for i, (query_message, response_message) in enumerate(zip(inputs.messages[::2], inputs.messages[1::2])):
            ...
            if "mask_start_tag" in inputs.extra_kwargs and "mask_end_tag" in inputs.extra_kwargs:
                mask_start_tag, mask_end_tag = inputs.extra_kwargs["mask_start_tag"], inputs.extra_kwargs["mask_end_tag"]
            else:
                mask_start_tag, mask_end_tag = None, None
            self._concat_context_list(
                ...
                system=system,
                round0=i,
                mask_start_tag=mask_start_tag,
                mask_end_tag=mask_end_tag)
    ...
    def replace_video2image(self, load_video_func, inputs, replace_tag: Callable) -> List[Context]:
        ...
        images = inputs.images
        try:
            new_images, timestamps = load_video_func(video)
        except:
        ...
        if timestamps is not None:
            for idx, i in enumerate(timestamps):
                # context_list += replace_tag(idx, i)
        ...
        # print(context_list)     # ['Frame1: ', '<image>\n', 'Frame2: ', '<image>\n', 'Frame3: ', '<image>\n', 'Frame4: ', '<image>\n', 'Frame5: ', '<image>\n', 'Frame6: ', '<image>\n', 'Frame7: ', '<image>\n', 'Frame8: ', '<image>\n']
        return context_list
    ...
    def _rlhf_data_collator(self,
        ...
        # 1 dict_keys(['chosen_input_ids', 'chosen_labels', 'chosen_pixel_values', 'chosen_length', 'rejected_input_ids', 'rejected_labels', 'rejected_pixel_values', 'rejected_length', 'length'])
        # 1 dict_keys(['chosen_input_ids', 'chosen_labels', 'chosen_pixel_values', 'chosen_length', 'rejected_input_ids', 'rejected_labels', 'rejected_pixel_values', 'rejected_length', 'length'])
        for prefix in [chosen_prefix, rejected_prefix]:
            new_batch += self._fetch_inputs_startswith(batch, prefix)
        # print(len(new_batch), new_batch[0].keys())
        # 2 dict_keys(['input_ids', 'labels', 'pixel_values', 'length'])   一个chosen 一个rejected
