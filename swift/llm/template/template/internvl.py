...
from ..vision_utils import load_video_internvl, load_video_internvl_self, transform_image
...
class Internvl2Template(InternvlTemplate):
    ...
    def init_env_args(self):
        ...
        self.self_load = get_env_args('self_load', bool, False)
        self.duration = get_env_args('duration', float, 4.0)
        self.target_fps = get_env_args('fps', int, 4)
        self.use_timestamp = get_env_args('use_timestamp', bool, False)
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            if self.self_load:
                load_video = partial(load_video_internvl_self, duration=self.duration, target_fps=self.target_fps,
                    use_timestamp=self.use_timestamp)
            else:
                load_video = partial(load_video_internvl, num_segments=self.video_segments)
            if self.use_timestamp:
                # return self.replace_video2image(load_video, inputs, lambda idx, i: [f'Frame{idx + 1} ({i} seconds): '] + image_context)
                return self.replace_video2image(load_video, inputs, lambda i: [f'{i} seconds: '] + image_context)
            else:
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        # print("&"*80)
        # print(encoded.keys())   # dict_keys(['input_ids', 'labels', 'loss_scale'])
        input_ids = encoded['input_ids']
        idx_list = findall(input_ids, -100)
        ...
        if images:
            ...
            aug_real_only = get_env_args('aug_real_only', bool, False)
            aug_lq_only = get_env_args('aug_lq_only', bool, False)
            aug_hq_only = get_env_args('aug_hq_only', bool, False)
            pre_resize = get_env_args('pre_resize', bool, False)
            pre_resize_size = get_env_args('pre_resize_size', int, None)
            pre_aug = get_env_args('pre_aug', bool, False)
            gaussian_sigma = get_env_args('gaussian_sigma', float, None)
            jpeg_quality = get_env_args('jpeg_quality', int, None)
            resize_robust = get_env_args('resize_robust', bool, False)
            if use_aug and self.is_training:
                if aug_real_only:
                    aug = True if inputs.label == 0 else False  # aug for real images, during training
                else:
                    aug = True
            else:
                aug = False
            pixel_values = [transform_image(
                image, self.input_size, max_num,
                aug=aug, pre_resize=pre_resize, pre_resize_size=pre_resize_size, aug_lq_only=aug_lq_only, aug_hq_only=aug_hq_only, pre_aug=pre_aug,
                gaussian_sigma=gaussian_sigma, jpeg_quality=jpeg_quality, resize_robust=resize_robust
            ) for image in images]
