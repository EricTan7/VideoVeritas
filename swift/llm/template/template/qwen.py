...
from .self_aug import flow_guided_deformation, motion_perturbation, fade_perturbation, local_temporal_shuffle, local_spatiotemporal_flicker
import functools
import numpy as np
try:
...
class Qwen2VLTemplate(Template):
    ...
    def init_env_args(self):
        ...
        self.flow_dir = get_env_args('FLOW_DIR', str, None)
    ...
    def replace_tag(self, media_type: Literal['image', 'video', 'audio'], index: int,
        ...
        if media_type == 'image':
            ...
            # ## MDPO aug
            # aug_func = None
            # self_aug = inputs.extra_kwargs["aug"] if "aug" in inputs.extra_kwargs else None
            # video_path = inputs.videos[0] if isinstance(inputs.videos, list) else inputs.videos
            # if self_aug is not None:
            #     if self_aug == "randomaug":
            #         assert "flow_index" in inputs.extra_kwargs
            #         # self_aug = np.random.choice(["deform", "motion", "fade"])
            #         self_aug = np.random.choice(["deform", "fade", "shuffle", "flicker", "motion"])
            #     if self_aug == "deform":
            #         assert "flow_index" in inputs.extra_kwargs
            #         flow_path = os.path.join(self.flow_dir, "_".join(video_path.split("/")[-3:]))
            #         flow_index = inputs.extra_kwargs["flow_index"]
            #         num = np.random.randint(1, len(flow_index)+1)
            #         strength = float(np.random.randint(15, 31))     # 20
            #         region = np.random.choice(["motion"]*4+["static"]*2+["any"]*2)   # 2/3 motion
            #         aug_duration = np.random.randint(2, 7)      # 4
            #         aug_func = functools.partial(flow_guided_deformation, num_deformations=num, strength=strength, duration=aug_duration, region=region, flow_path=flow_path, flow_index=flow_index)
            ...
            #         region = np.random.choice(["motion"]*4+["static"]*2+["any"]*2)   # 2/3 motion
            #         aug_func = functools.partial(motion_perturbation, strength=strength, num_perturbations=num, flow_path=flow_path, flow_index=flow_index)
            #     elif self_aug == "fade":
            #         aug_duration = np.random.randint(2, 7)
            #         num = np.random.randint(1, 4)
            #         aug_func = functools.partial(fade_perturbation, num_events=num, patch_size_ratio_range=(0.1, 0.2), fade_duration=aug_duration)
            #     # simple shuffle/flicker
            #     elif self_aug == "shuffle":
            #         chunk_size = np.random.randint(2, 5)
            #         num = 1
            #         aug_func = functools.partial(local_temporal_shuffle, num_shuffled_chunks=num, chunk_size=chunk_size)
            #     elif self_aug == "flicker":
            #         chunk_size_t = np.random.randint(2, 5)
            #         chunk_size_h = np.random.randint(32, 64)
            #         chunk_size_w = np.random.randint(32, 64)
            #         num = np.random.randint(1, 4)
            #         aug_func = functools.partial(local_spatiotemporal_flicker, num_flickers=num, cube_size=(chunk_size_t, chunk_size_h, chunk_size_w))
            #     else:
            ...
            # if aug_func is None:
            #     self_aug = inputs.extra_kwargs["grpo_aug"] if "grpo_aug" in inputs.extra_kwargs else None
            #     video_path = inputs.videos[0] if isinstance(inputs.videos, list) else inputs.videos
            #     if self_aug is not None:
            #         if self_aug == "randomaug":
            #             assert "flow_index" in inputs.extra_kwargs
            #             self_aug = np.random.choice(["deform", "fade", "flicker"])  # only spatiotemporal in GRPO
            #         if self_aug == "deform":
            #             assert "flow_index" in inputs.extra_kwargs
            #             flow_path = os.path.join(self.flow_dir, "_".join(video_path.split("/")[-3:]))
            #             flow_index = inputs.extra_kwargs["flow_index"]
            ...
            #             region = np.random.choice(["motion"]*4+["static"]*2+["any"]*2)   # 2/3 motion
            #             aug_duration = np.random.randint(2, 7)      # 4
            #             aug_func = functools.partial(flow_guided_deformation, num_deformations=num, strength=strength, duration=aug_duration, region=region, flow_path=flow_path, flow_index=flow_index, return_aug_info=True)
            #         elif self_aug == "fade":
            #             aug_duration = np.random.randint(2, 7)
            #             num = 1
            #             aug_func = functools.partial(fade_perturbation, num_events=num, patch_size_ratio_range=(0.1, 0.2), fade_duration=aug_duration, return_aug_info=True)
            #         elif self_aug == "flicker":
            #             chunk_size_t = np.random.randint(2, 5)
            #             chunk_size_h = np.random.randint(32, 64)
            #             chunk_size_w = np.random.randint(32, 64)
            #             num = 1
            #             aug_func = functools.partial(local_spatiotemporal_flicker, num_flickers=num, cube_size=(chunk_size_t, chunk_size_h, chunk_size_w), return_aug_info=True)
            #         else:
            #             raise ValueError(self_aug)
            #         return_aug_info = True

            # if return_aug_info:
            #     video, video_kwargs, aug_info = fetch_video(video_inputs, return_video_sample_fps=True, aug_funcs=aug_func, return_aug_info=return_aug_info, **kwargs)
            # else:
            #     video, video_kwargs = fetch_video(video_inputs, return_video_sample_fps=True, aug_funcs=aug_func, return_aug_info=return_aug_info, **kwargs)
            # video, video_kwargs = fetch_video(video_inputs, return_video_sample_fps=True, aug_funcs=aug_func, **kwargs)

            video_inputs['task'] = inputs.extra_kwargs.get("task", None)
    ...
    def _data_collator_mm_data(self, batch: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        #         if b["extra"].get("aug") is not None:
        #             res_aug.append(b["extra"]["aug"])
        #         else:
        #             res_aug.append(None)
        # if res_aug:
        #     res["aug"] = res_aug
        ...
        # for b in batch:
        #     if b.get("extra") is not None:
        #         if b["extra"].get("use_nll_loss") is not None:
        #             res_use_nll_loss.append(b["extra"]["use_nll_loss"])
        #         else:
        #             res_use_nll_loss.append(None)
        # if res_use_nll_loss:
        #     res["use_nll_loss"] = res_use_nll_loss

        # res_aug_info = []
        # for b in batch:
        #     if b.get("aug_info") is not None:
        #         res_aug_info.append(b["aug_info"])
        #     else:
        #         res_aug_info.append(None)
        # if res_aug_info:
        #     res["aug_info"] = res_aug_info
        return res
...
class Qwen3VLTemplate(Qwen2VLTemplate):
    ...
    def _encode(self, inputs: StdTemplateInputs) -> Dict[str, Any]:
        ...
        encoded['labels'] = labels
        encoded['loss_scale'] = loss_scale
        encoded["extra"] = inputs.extra_kwargs
        # encoded["aug_info"] = inputs.aug_info
        return encoded
