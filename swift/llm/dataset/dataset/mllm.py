...
from .data_utils import DATASET_FACTORY_SHAREGPT, DATASET_FACTORY_SHAREGPT_TEST
...
### ============ custom dataset
## support video & image
class CustomPreprocessor_Reason_SFT(MessagesPreprocessor):
    def preprocess(self, row: Dict[str, Any]) -> Optional[Dict[str, Any]]:
        row = super().preprocess(row)
...
for dataset_name, dataset_path in DATASET_FACTORY_SHAREGPT.items():
    register_dataset(
        DatasetMeta(
            dataset_path=dataset_path,
            dataset_name=dataset_name+"_reason",
            preprocess_func=CustomPreprocessor_Reason_SFT(),
        ))
...
for dataset_name, dataset_path in DATASET_FACTORY_SHAREGPT.items():
    register_dataset(
        DatasetMeta(
            dataset_path=dataset_path,
            dataset_name=dataset_name+"_reason_rl",
            preprocess_func=CustomPreprocessor_Reason_RL(),
        ))


### Test set
for dataset_name, dataset_path in DATASET_FACTORY_SHAREGPT_TEST.items():
    register_dataset(
        DatasetMeta(
            dataset_path=dataset_path,
            dataset_name=dataset_name+"_reason",
...
for dataset_name, dataset_path in DATASET_FACTORY_SHAREGPT_TEST.items():
    register_dataset(
        DatasetMeta(
            dataset_path=dataset_path,
            dataset_name=dataset_name+"_video",
            preprocess_func=CustomPreprocessor_Reason_test(),
        ))
