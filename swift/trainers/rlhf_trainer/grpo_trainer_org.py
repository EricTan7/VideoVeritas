...
class PairedRepeatSampler(Sampler):
    """
    配对任务的重复采样器（适配GRPO）
    ...
    Args:
        ...
        batch_size: 每个batch的样本数（必须是偶数，因为任务成对）
        repeat_count: batch级别的重复次数
        shuffle: 是否打乱
        seed: 随机种子
    
    Example:
        假设有3个视频，每个视频2个任务，batch_size=4（2个视频），
        ...
        输出索引序列（shuffle=False）:
        # Batch 1 (video_0, video_1)
        [0, 0, 1, 1, 2, 2, 3, 3,  # repeat 1
         0, 0, 1, 1, 2, 2, 3, 3]  # repeat 2
        # Batch 2会被drop（只有1个视频，不足batch_size=4）
    """
    ...
    def __init__(
        ...
        # batch_size必须是偶数（每个视频2个任务）
        if batch_size % 2 != 0:
            raise ValueError(
                f"batch_size必须是偶数（每个视频有2个任务），当前为 {batch_size}"
            )
        
        # 构建视频配对映射
        self.video_pairs = self._build_video_pairs()
        self.num_videos = len(self.video_pairs)
        self.videos_per_batch = batch_size // 2
        ...
        print(f"PairedRepeatSampler 初始化完成:")
        print(f"  数据集大小: {len(data_source)}")
        print(f"  配对视频数: {self.num_videos}")
        print(f"  batch_size: {batch_size} (对应{self.videos_per_batch}个视频)")
        print(f"  mini_repeat_count: {mini_repeat_count}")
        print(f"  repeat_count: {repeat_count}")
        print(f"  有效batch数: {self.num_videos // self.videos_per_batch}")
    
    def _build_video_pairs(self) -> list:
        """
        构建视频配对列表
        返回: [(video_id, aigc_idx, st_idx), ...]
        """
        ...
        for idx in range(len(self.data_source)):
            ...
            task = sample['task']
            video_dict[video_id][task] = idx
        
        # 构建完整配对
        pairs = []
        incomplete_videos = []
        
        for video_id, tasks in video_dict.items():
            aigc_idx = tasks.get('aigc')
            st_idx = tasks.get('counting')
            
            if aigc_idx is not None and st_idx is not None:
                pairs.append((video_id, aigc_idx, st_idx))
            else:
                incomplete_videos.append(video_id)
        
        if incomplete_videos:
            print(f"⚠️  警告: {len(incomplete_videos)}个视频缺失任务配对，已忽略")
            if len(incomplete_videos) <= 5:
                print(f"    示例: {incomplete_videos}")
    ...
    def __iter__(self):
        ...
        逻辑（遵循官方RepeatSampler）：
        1. 打乱（或不打乱）视频索引（只执行一次）
        2. 按batch_size分组视频
        3. 丢弃不完整的batch
        4. 对每个batch:
           4.1 重复repeat_count次
           4.2 对batch内每个视频:
               4.2.1 输出AIGC任务索引（重复mini_repeat_count次）
               4.2.2 输出ST任务索引（重复mini_repeat_count次）
        """
        # Step 1: 获取视频索引并打乱（只执行一次）
        if self.shuffle:
            video_indexes = torch.randperm(
                self.num_videos, 
                generator=self.generator
            ).tolist()
        else:
            video_indexes = list(range(self.num_videos))
        
        # Step 2: 按videos_per_batch分组
        # [[0, 1], [2, 3], [4, 5], ...]
        video_chunks = [
            video_indexes[i : i + self.videos_per_batch]
            for i in range(0, len(video_indexes), self.videos_per_batch)
        ]
        ...
        for chunk in video_chunks:  # 对每个batch
            for _ in range(self.repeat_count):  # 重复整个batch
                for video_idx in chunk:  # batch内的每个视频
                    video_id, aigc_idx, st_idx = self.video_pairs[video_idx]
...
class GRPOTrainer(RolloutTrainerMixin, SwiftMixin, HFGRPOTrainer):
    ...
    def _get_train_sampler(self, train_dataset=None):
        use_paired = get_env_args('use_paired_sampler', bool, False)
    ...
    def _compute_advantages(self, inputs: DataType, rewards_per_func: torch.Tensor,
        ...
        # ## ====== Mixed Group Mode ======
        # use_paired = get_env_args('use_paired_sampler', bool, False)
        # paired_advantage_mode = get_env_args('paired_advantage_mode', str, None)
        # if use_paired:
        #     assert paired_advantage_mode is not None, "can't use paired sampler without paired_advantage_mode"
        #     if self.paired_advantage_mode == 'independent':
        #         # 标准GRPO：每个任务独立计算
        #         advantages = self._compute_independent_advantages(
        #             rewards, video_task_structure
        #         )
        #     elif self.paired_advantage_mode == 'coupled':
        #         # 耦合模式：同一视频的任务一起考虑
        #         advantages = self._compute_coupled_advantages(
        #             rewards, rewards_per_func, video_task_structure
        #         )
        #     elif self.paired_advantage_mode == 'conditional':
        #         # 条件模式：一个任务影响另一个
        #         advantages = self._compute_conditional_advantages(
        #             rewards, rewards_per_func, video_task_structure
        #         )
        #     else:
        #         raise ValueError(f"Unknown task_advantage_mode: {self.task_advantage_mode}")
