...
class BoxBinaryAcc(ORM):
    def __init__(self, alpha=0.05, beta=0.05):
        self.alpha = alpha
        self.beta = beta
        self.epsilon = 1e-6

    def _correctness_fn(self, completion_text, solution):
        if solution in ["0", 0]:
        ...
        if student_answer not in ["fake", "real"]:
            return False
...
class AnswerFormat(ORM):
    def __call__(self, completions, **kwargs) -> List[float]:
        ...
        for content in completions:
            if bool(re.match(pattern, content, re.DOTALL)):
                rewards.append(1.0)
            else:
                rewards.append(0.0)
        return rewards
...
class Counting(ORM):
    ...
    def _get_correctness_and_lengths(self, completions: List[List[str]], solution: List[List[str]]):
        ...
        for k in range(len(completions)):
            ...
            try:
                predicted_counts = [int(c.strip()) for c in completion_text.split(',')]
                ground_truth_counts = [int(s.strip()) for s in solution_text.split(',')]
            ...
            for y_hat, y in zip(predicted_counts, ground_truth_counts):
                absolute_error = abs(y_hat - y)
                
                denominator = y + epsilon

                score_k = max(0.0, 1.0 - absolute_error / denominator)
                category_scores.append(score_k)

            r_naive.append(np.mean(category_scores))

        return r_naive

    def __call__(self, completions, **kwargs) -> List[float]:
        solution = kwargs["counts"]
        r_naive = self._get_correctness_and_lengths(completions, solution)
...
def calculate_spatiotemporal_iou(pred_time_span, pred_boxes, gt_time_span, gt_boxes):
    pred_t1, pred_t2 = pred_time_span
    gt_t1, gt_t2 = gt_time_span
    
    inter_t = max(0, min(pred_t2, gt_t2) - max(pred_t1, gt_t1))
    union_t = (pred_t2 - pred_t1) + (gt_t2 - gt_t1) - inter_t
    temporal_iou = inter_t / union_t if union_t > 0 else 0
    
    matched_times = set(pred_boxes.keys()) & set(gt_boxes.keys())
    if matched_times:
        spatial_ious = [
            calculate_iou(pred_boxes[t], gt_boxes[t]) 
            for t in matched_times
        ]
        spatial_iou = sum(spatial_ious) / len(spatial_ious)
    else:
        spatial_iou = 0
    
    return temporal_iou, spatial_iou



class TrackingIoU(ORM):
    def __init__(self, alpha=0.05, beta=0.05):
        self.alpha = alpha
        self.beta = beta
        self.epsilon = 1e-6
        
    def _get_correctness_and_lengths(self, completions: List[List[str]], kwargs):
        r_naive = []
        gt_boxes_all = kwargs["gt_track_boxes"]
        for k in range(len(completions)):
            completion_text = completions[k]
            gt_boxes = gt_boxes_all[k]
            ...
            if gt_boxes is None:
                ...
                try:
                    ...
                    minlen = min(len(bboxes), len(gt_boxes))
                    iou_all = [calculate_iou(bboxes[i], gt_boxes[i]) for i in range(minlen)]
                    r_naive.append(np.mean(iou_all))
                except:
                    r_naive.append(0.0)
        return r_naive

    def __call__(self, completions, **kwargs) -> List[float]:
        r_naive = self._get_correctness_and_lengths(completions, kwargs)
...
class STGIoU(ORM):
    ...
    def _get_correctness_and_lengths(self, completions: List[List[str]], kwargs):
        r_naive = []
        gt_time_all, gt_boxes_all = kwargs["gt_time"], kwargs["gt_stg_boxes"]
        for k in range(len(completions)):
            completion_text = completions[k]
            gt_time, gt_boxes = gt_time_all[k], gt_boxes_all[k]

            if gt_boxes is None:
                r_naive.append(0.0)
            else:
                try:
                    gt_boxes = {k: v for k, v in gt_boxes.items() if v is not None}
                    pred = json.loads(completion_text)
                    pred_time, pred_boxes = pred["time"], pred["boxes"]
                    if len(pred_time) != 2:
                        r_naive.append(0.0)
                        continue
                    tiou, siou = calculate_spatiotemporal_iou(pred_time, pred_boxes, gt_time, gt_boxes)
                    iou_score = (tiou+siou) / 2
                    r_naive.append(iou_score)
                except:
                    r_naive.append(0.0)
...
class MCQAcc(ORM):
    ...
    def _get_correctness_and_lengths(self, completions: List[List[str]], kwargs):
        r_naive = []
        gt_options = kwargs["gt_option"]
        for k in range(len(completions)):
            completion_text = completions[k]
            try:
                gt_option = gt_options[k].strip()
                
                if gt_option is None:
                    r_naive.append(0.0)
                else:
                    pred_option = completion_text.strip()
...
def calculate_point_reward(pred_str, gt_centers, max_boxes=4, contain=True):
    """计算预测bbox的奖励"""
    boxes = json.loads(pred_str)["boxes"]
    n_gt = len(gt_centers)
    ...
    if len(boxes) == 0 or n_gt == 0:
        return 0.0
    
    max_area = 1000 * 1000 / 9
    ...
    n_pred = len(valid_boxes)
    cost_matrix = np.zeros((n_pred, n_gt))
    max_dist = np.sqrt(2 * 200**2)
        ...
        for j, gt in enumerate(gt_centers):
            ...
            if contain:
                if x1 <= gx <= x2 and y1 <= gy <= y2:
                    ...
                    dist = np.sqrt((box_center[0] - gx)**2 + (box_center[1] - gy)**2)
                    dist_reward = 1.0 - min(dist / max_dist, 1.0)
                    reward = reward + contain_reward * 0.2 + dist_reward * 0.8
            else:
                dist = np.sqrt((box_center[0] - gx)**2 + (box_center[1] - gy)**2)
                dist_reward = 1.0 - min(dist / max_dist, 1.0)
                reward = reward + dist_reward * 1.0
    ...
    if matched_count > 0:
        normalized_reward = total_reward / matched_count
    else:
...
class PointRewardContain(ORM):
    ...
    def _get_correctness_and_lengths(self, completions: List[List[str]], kwargs):
        r_naive = []
        gt_points_all = kwargs["gt_points"]
        for k in range(len(completions)):
            completion_text = completions[k]
            gt_points = gt_points_all[k]

            if not gt_points:
                r_naive.append(0.0)
            else:
                try:
                    reward = calculate_point_reward(completion_text, gt_points, contain=True)
                    r_naive.append(reward)
                except:
                    r_naive.append(0.0)
        return r_naive
...
class PointReward(ORM):
    ...
    def _get_correctness_and_lengths(self, completions: List[List[str]], kwargs):
        r_naive = []
        gt_points_all = kwargs["gt_points"]
        for k in range(len(completions)): # 遍历扁平化列表
            completion_text = completions[k]
            gt_points = gt_points_all[k]
            ...
            if not gt_points:
                ...
                try:
                    reward = calculate_point_reward(completion_text, gt_points, contain=False)
                    r_naive.append(reward)
                except:
                    r_naive.append(0.0)
        return r_naive
...
orms['box_bin_acc'] = BoxBinaryAcc
orms['answer_format'] = AnswerFormat

orms['counting'] = Counting
orms['tracking_iou'] = TrackingIoU
orms['stg_iou'] = STGIoU
orms['mcq'] = MCQAcc
orms['point_contain'] = PointRewardContain
orms['point'] = PointReward
